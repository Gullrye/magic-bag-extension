...
const zhCN: MessageCatalog = {
  ...
  popupSyncEyebrow: '备份',
  popupSyncTitle: '本地备份',
  popupSyncBound: (params) => `当前备份：${params?.fileName ?? ''}`,
  popupSyncUnbound: '还没有本地备份文件',
  popupSyncHint: '选择一个保存位置后，法宝袋会自动创建备份文件。',
  popupSyncBind: '开启本地备份',
  popupSyncImport: '从备份恢复',
  popupSyncExport: '更新本地备份',
  popupSyncBindSuccess: (params) => `已开启本地备份：${params?.fileName ?? ''}`,
  popupSyncImportSuccess: (params) => `已从本地备份恢复 ${params?.count ?? 0} 个标签页`,
  popupSyncExportSuccess: (params) => `已更新本地备份，共 ${params?.count ?? 0} 个标签页`,
  popupSyncNeedBind: '请先开启本地备份',
  popupSyncUnsupported: '当前环境不支持本地备份文件',
  popupSyncPermissionDenied: '没有获得本地备份文件权限',
  popupSyncInvalidFile: '本地备份文件内容无效，请检查 JSON 格式',
  popupSyncActionError: '本地备份操作失败',
  popupSyncChangeFile: '更换备份文件',
...
const enUS: MessageCatalog = {
  ...
  popupLanguageZh: '中文',
  popupLanguageEn: 'EN',
  popupTooltipHint: 'Hint',
  popupSyncEyebrow: 'Backup',
  popupSyncTitle: 'Local Backup',
  popupSyncBound: (params) => `Current backup: ${params?.fileName ?? ''}`,
  popupSyncUnbound: 'No local backup file yet',
  popupSyncHint: 'Choose a save location once and Magic Bag will create the backup file for you.',
  popupSyncBind: 'Enable Local Backup',
  popupSyncImport: 'Restore from Backup',
  popupSyncExport: 'Update Local Backup',
  popupSyncBindSuccess: (params) => `Local backup enabled: ${params?.fileName ?? ''}`,
  popupSyncImportSuccess: (params) => {
    const count = Number(params?.count ?? 0);
    return `Restored ${count} tab${count === 1 ? '' : 's'} from backup`;
  },
  popupSyncExportSuccess: (params) => {
    const count = Number(params?.count ?? 0);
    return `Updated local backup with ${count} tab${count === 1 ? '' : 's'}`;
  },
  popupSyncNeedBind: 'Enable local backup first',
  popupSyncUnsupported: 'This environment does not support local backup files',
  popupSyncPermissionDenied: 'Permission to access the local backup file was denied',
  popupSyncInvalidFile: 'The local backup file contents are invalid',
  popupSyncActionError: 'Local backup action failed',
  popupSyncChangeFile: 'Change Backup File',
  ...
  popupExportButton: 'Export Tabs',
  popupIoTitle: 'Backup & Archive',
  popupIoHint: 'Export saves all tabs as a JSON file; Import restores tabs from a JSON file.',
